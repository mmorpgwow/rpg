...
public class RenderBattleGround {
    ...
    public void update(String activePlayer, int roundTime, List<Ranged> rangedSkillList) {
        initGrid(initChar);

        initBattleGround((int) (RenderHeroInfo.GRID_SIZE_X * 2 - xBG) / 2, rangedSkillList);
    ...
    private void initBattleGround(int x, List<Ranged> rangedSkillList) {
        ...
        if (!rangedSkillList.isEmpty()) {
            for (Ranged s : rangedSkillList) {
                if (s.getIsCasted()) {
                    if (s.getYPos() < yBG && s.getXPos() < xBG &&
                            s.getYPos() > 0 && s.getXPos() > 0) {
                        this.grid[s.getYPos()][s.getXPos() + x] = skillSymbol;
                    }
                }
            }
        }
